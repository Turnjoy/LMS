...
def _marketing_domains(app):
    ...
    base_domains = [
        ...
        'www.turnjoy.com', 
        'turnjoylms.com.ng', 
        'www.turnjoylms.com.ng',
        'lms-a5n7.onrender.com', 
        'turnjoy-lms.onrender.com', 
    ...
    if configured:
        if isinstance(configured, str):
            base_domains.extend(configured.split(','))
        elif isinstance(configured, list):
            base_domains.extend(configured)
            
    normalized = {_normalize_host(domain) for domain in base_domains if domain}
...
def _subdomain_from_host(host, app):
    """Return a tenant subdomain candidate for multi-label hosts, avoiding marketing domain collisions."""
    if not host or host in _local_dev_domains():
        return None
    ...
    def set_tenant_context():
        ...
        try:
            ...
            if is_master_path and not is_marketing_host:
                print(f"[ROUTING LOG] Blocked Master Admin Path context: Host '{host}' is not a marketing host.")
                abort(404)

            if is_marketing_host:
                print(f"[ROUTING LOG] Host '{host}' successfully matched marketing. Routing normally.")
                session.pop('current_tenant', None)
                return

            if host not in _local_dev_domains():
                print(f"[ROUTING LOG] Non-marketing host '{host}' detected. Commencing database custom_domain lookup...")
                tenant = Tenant.query.filter_by(custom_domain=host).first()

                subdomain = _subdomain_from_host(host, app)
                print(f"[ROUTING LOG] Parsed Subdomain Candidate: '{subdomain}'")
                if not tenant and subdomain:
                    print(f"[ROUTING LOG] Commencing database subdomain lookup for '{subdomain}'...")
                    tenant = Tenant.query.filter_by(subdomain=subdomain).first()

            if not tenant:
                print(f"[ROUTING LOG] No tenant matched for Host '{host}'. Paths checked: {request.path}")
                if request.path in marketing_only_paths:
                    print(f"[ROUTING LOG] Hard-aborting with 404. Host '{host}' is not authorized to access marketing paths.")
                    abort(404)

            if tenant:
                print(f"[ROUTING LOG] Successfully matched Tenant '{tenant.name}' (ID: {tenant.id})")
                if not _tenant_is_active(tenant):
                    print(f"[ROUTING LOG] Tenant '{tenant.name}' is inactive. Routing to Lockout page.")
                ...
                if (
                    ...
                    print(f"[ROUTING LOG] User tenant mismatch. Logging out user.")
                    logout_user()
                    return render_template('public/lockout.html', tenant=tenant), 403
                ...
                if request.path == '/':
                    print(f"[ROUTING LOG] Tenant resolving fell through entirely for path '/'. Aborting 404.")
                    abort(404)
        except HTTPException:
            raise
        except Exception as e:
            print(f"[ROUTING CRITICAL EXCEPTION] Error: {str(e)}", file=sys.stderr)
            if request.path.startswith('/turnjoy-master-admin') or request.path.startswith('/_master_hq_2026'):
                abort(404)
            g.from flask import Flask, abort, g, redirect, render_template, request, session, url_for
from flask_login import LoginManager
from werkzeug.exceptions import HTTPException
from werkzeug.middleware.proxy_fix import ProxyFix
from config import config
